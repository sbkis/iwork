...
directory (`IGLOO_DIR`, default `~/dev/igloo`). Worktrees are collected under an
`AAWORKTREES/` sibling:

```
~/dev/igloo/                         # IGLOO_DIR
├── rent-api/                        # your repos (real git repos, direct children)
├── auth-api/
└── AAWORKTREES/                     # task folders live here
    └── feat-login/                  # one task folder
        ├── auth-api--feat-login/    # a git worktree (named repo--branch-slug)
        ├── rent-api--feat-login/    # another worktree, same branch
        ├── CLAUDE.md                # generated task context (scopes the agent)
        └── AGENTS.md                # identical content, for Codex
```

The tool itself can live anywhere — it does **not** need to be under `IGLOO_DIR`.
...
```bash
git clone https://github.com/sbkis/iwork.git ~/dev/igloo/iwork
```

Clone it wherever you like; the path above just keeps it alongside your repos.

### 2. Put `iwork` on your PATH

Symlink the script into a directory already on your `PATH` (`~/.local/bin` works
on this machine):

```bash
ln -s ~/dev/igloo/iwork/iwork ~/.local/bin/iwork
```

If `~/.local/bin` is not on your `PATH`, add `export PATH="$HOME/.local/bin:$PATH"`
to your shell rc.

### 3. Enable shell integration (recommended)
...
### 4. Install Claude Code status hooks (optional)
...
### 5. Recommended tmux config (optional)

Task windows live in a dedicated `tasks` session. These `~/.tmux.conf` additions make
it easy to move between your repo session and the tasks session, and surface how many
agents are waiting:
...
bind I switch-client -t igloo       # prefix+I: jump to your repo session
...
iwork feat/login-bug -r auth-api rent-frontend-v2

# Add another repo to that task later
iwork add-repo feat-login-bug -r accounting-api
...
|---|---|---|
| `IGLOO_DIR` | `~/dev/igloo` | Where your repos (and `AAWORKTREES/`) live |
...
For the design rationale behind the tmux windows, status hooks, and task context
files, see the specs and implementation plans under [`docs/superpowers/`](docs/superpowers/).
